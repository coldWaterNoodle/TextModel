...
// Content 필드 스키마
const CONTENT_FIELD_SCHEMA = {
    name: 'Content',
    type: 'multilineText',
};

async function addContentField() {
    console.log('🔧 Content 필드 추가 중...');
    ...
    try {
        ...
        const table = tables.find(t => t.name === 'Medicontent Posts');
        if (!table) {
            throw new Error('Medicontent Posts 테이블을 찾을 수 없습니다.');
        }
        
        // Content 필드가 이미 있는지 확인
        const existingField = table.fields.find(f => f.name === 'Content');
        if (existingField) {
            console.log('✅ Content 필드가 이미 존재합니다.');
            return;
        }
        ...
        const addFieldResponse = await fetch(`${METADATA_API_URL}/${table.id}/fields`, {
            method: 'POST',
            headers: HEADERS,
            body: JSON.stringify(CONTENT_FIELD_SCHEMA),
        });
        
        if (!addFieldResponse.ok) {
            throw new Error(`Failed to add Content field: ${await addFieldResponse.text()}`);
        }
        
        console.log('✅ Content 필드 추가 완료');
        
    } catch (error) {
        console.error('❌ Content 필드 추가 실패:', error.message);
    }
}
...
async function main() {
    console.log('🚀 Content 필드 추가 및 포스트 콘텐츠 저장을 시작합니다...\n');
    ...
    try {
        ...
        console.log('\n🎉 Content 필드 추가 및 포스트 콘텐츠 저장이 완료되었습니다!');
